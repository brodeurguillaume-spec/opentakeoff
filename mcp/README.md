...
| `measure_polygon` | Area + perimeter of a polygon you supply (min 3 verts). Requires scale. A committed polygon is unreviewed agent pencil until a person affirms it. |
...
| `measure_line` | Length of an open polyline (min 2 points). Requires scale. A committed line is unreviewed agent pencil until a person affirms it. |
...
| `measure_surface` | **Wall SF**: an open run traced along the wall, quantified as traced LF × the condition's height (the canvas's H knob — pass `height_ft` to set it, or set it once with `edit_condition`). Wall tile, wainscot, wall systems. Refuses without a height, minting nothing; a committed run is unreviewed agent pencil until a person affirms it. |
| `place_count` | **EA markers**: one point, one each — thresholds, stair nosings, floor boxes. No scale required (EA is scale-free). One unreviewed agent-pencil shape per point; the whole call is one undo step. |
...
| `export_takeoff` | The full `opentakeoff.takeoff_canvas.v1` payload — exactly what the app autosaves, including additive persistent project-map `regions` when present. Inline, and to disk with `path` (see **Writing to disk** below). |
...
| `import_takeoff` | **The way back in**: load a `takeoff_canvas.v1` file (a prior `export_takeoff`, or the app's own save) through the SAME merge rules as the app's Sheet-menu import — finish-tag identity joins conditions, stable-id project-map regions merge operator-first, new ids append, duplicates skip (idempotent re-import), this session's calibration wins per sheet. Resume, extend, or audit. Correction rules (#88) ride the file too — `apply_rules` re-runs them. |
...
- A human-confirmed Project Map region with `purposes: ["scale"]` and a
  `scale_profile.units_per_px` overrides the sheet fallback only for geometry
  wholly inside it. Nested children override parents. Cross-boundary geometry,
  unconfirmed or non-uniform profiles, and conflicting overlaps refuse before
  mutation; scaled replies cite `scale_source` and `scale_region_id`.
- `detect_rooms` is a whole-sheet sweep and currently refuses a sheet carrying
  scale zones; use `one_click` or `measure_polygon` inside each confirmed zone.
