...
export const exportTakeoffOutput = {
  ...
  shapes: z.array(z.object({
    ...
    measure_role: z.enum(["floor_area", "deduct", "linear", "surface_area", "count", "count_run"]),
    verts_norm: z.array(point).describe("Vertices normalized to sheet dims (0–1)"),
    computed: z.object({ area_sf: z.number().optional(), perimeter_lf: z.number().optional(), count: z.number().optional(), guide_lf: z.number().optional(), unit_length_in: z.number().optional(), joint_in: z.number().optional(), nominal_total_in: z.number().optional(), installed_span_in: z.number().optional() }).passthrough()
  ...
  sheet_group_layout: z.enum(["row", "column"]).optional().describe("Operator canvas layout for a multi-sheet group; omitted means side-by-side row"),
...
export const listShapesOutput = {
  shapes: z.array(z.object({
    ...
    guide_lf: z.number().optional(),
    unit_length_in: z.number().optional(),
    joint_in: z.number().optional(),
    installed_span_in: z.number().optional(),
