...
const SCALE_ZONES = "Confirmed Project Map scale zones override the sheet fallback. Geometry must stay wholly inside one compatible scale context; a boundary crossing, conflicting overlap, unconfirmed zone, or unsupported X/Y scale refuses before mutation. Replies cite scale_source and scale_region_id when regional.";
...
export function registerTools(realServer: McpServer, session: Session): Map<string, RegisteredTool> {
  ...
  server.registerTool("measure_polygon", {
    description: `Measure a closed polygon you supply (min 3 vertices, image px): area_sf and perimeter_lf at the applicable confirmed zone scale or sheet fallback. Pass condition to commit it as unreviewed agent pencil; role "deduct" subtracts. ${SCALE_ZONES} ${COORDS}`,
    inputSchema: {
      sheet: z.string(),
  ...
  server.registerTool("measure_line", {
    description: `Measure an open polyline (min 2 points, image px): length_lf at the applicable confirmed zone scale or sheet fallback. Pass condition to commit it as an unreviewed agent-pencil linear shape (base, transitions, feature strips). ${SCALE_ZONES} ${COORDS}`,
    inputSchema: {
      sheet: z.string(),
